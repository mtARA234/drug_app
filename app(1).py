...
# Google Drive path
DRIVE_PATH = "/content/drive/MyDrive/drug_models"
...
for name in targets:
    try:
        reg = XGBRegressor()
        reg.load_model(os.path.join(DRIVE_PATH, f"{name}_reg.json"))
        clf = XGBClassifier()
        clf.load_model(os.path.join(DRIVE_PATH, f"{name}_clf.json"))
        hybrid_models[name] = (reg, clf)
    except Exception as e:
        print(f"Skipping {name}: {e}")

tox_model = XGBClassifier()
tox_model.load_model(os.path.join(DRIVE_PATH,"tox_model.json"))

compat_model = XGBClassifier()
compat_model.load_model(os.path.join(DRIVE_PATH,"compatibility_xgb.json"))
